...
double BehaviorPlannerFSM::get_look_ahead_distance(const State& ego_state) {
  ...
  // Vf is zero, v initial is = velocity_mag, equation is vf*vf - vInit*viint/(2*acceleration)
  auto look_ahead_distance = std::abs((velocity_mag*velocity_mag)/(2*accel_mag));
